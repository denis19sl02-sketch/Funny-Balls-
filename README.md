...
<body>
<!-- РЕАЛЬНАЯ ПЛАТЕЖНАЯ ФОРМА С ВВОДОМ КАРТЫ -->
<div id="paymentModal">
  <div class="payment-container">
    <div class="payment-header">💳 ОПЛАТА $35</div>
    <p style="text-align:center;color:#7f8c8d;margin-bottom:10px;">Разблокировка уровней 15-35</p>
    ...
    <form id="cardForm">
      ...
      <div class="card-row">
        <div class="card-col">
          <label class="card-input-label">Срок действия</label>
          <input type="text" id="cardExpiry" class="card-input" placeholder="ММ/ГГ" maxlength="5">
        </div>
        <div class="card-col">
      ...
      <div class="card-input-group">
        <label class="card-input-label">Имя на карте</label>
        <input type="text" id="cardName" class="card-input" placeholder="IVAN IVANOV">
      </div>
      
      <button type="submit" id="submitPayment">💳 ОПЛАТИТЬ $35</button>
    </form>
    
    <div id="paymentStatus" class="payment-status"></div>
    
    <button id="closePayment">✖ ОТМЕНА</button>
    
    <div style="font-size:12px;color:#95a5a6;margin-top:20px;text-align:center;">
      <p>💳 Система проверяет карты через Stripe API</p>
      <p>✅ Реальные карты: списывается $35</p>
      <p>❌ Неправильные карты: ошибка</p>
      <p>🔒 Оплата защищена PCI DSS</p>
    </div>
  </div>
</div>
...
<script>
/* ============================================
   РЕАЛЬНАЯ ПЛАТЕЖНАЯ СИСТЕМА - ПРОВЕРКА КАРТ ЧЕРЕЗ STRIPE API
   ============================================ */
// 🔥 ВАШ ПУБЛИЧНЫЙ КЛЮЧ STRIPE (ЗАМЕНИТЕ НА СВОЙ!)
...
// Ваш бэкенд сервер (обязательно нужен!)
const BACKEND_URL = 'https://node-8080.csb.app'; // Ваш сервер

// Конфигурация платежа
const PAYMENT_CONFIG = {
  amount: 3500, // $35.00 в центах
  currency: 'usd',
...
/* ============================================
   ОБРАБОТЧИКИ ВВОДА ДАННЫХ КАРТЫ
   ============================================ */
...
/* ============================================
   РЕАЛЬНАЯ ФУНКЦИЯ ОПЛАТЫ ЧЕРЕЗ STRIPE API
   ============================================ */
async function processRealPayment() {
  try {
    // Валидация формы
    if (!validateForm()) {
      showPaymentStatus('❌ Проверьте правильность данных карты', 'error');
      return;
    }
    
    // Показываем статус обработки
    showPaymentStatus('🔐 Проверка карты через Stripe...', 'processing');
    submitPayment.disabled = true;
    submitPayment.textContent = 'ПРОВЕРКА...';
    ...
    // Отправляем запрос на ваш сервер для проверки карты
    ...
    if (result.success) {
      // Карта валидна, создаем платеж
      showPaymentStatus('✅ Карта проверена. Создание платежа...', 'processing');
      ...
      if (paymentResult.success) {
        // Платеж успешен!
        showPaymentStatus('✅ Платеж $35 успешно завершен!', 'success');
        ...
        throw new Error(paymentResult.error || 'Ошибка платежа');
      }
    } else {
      throw new Error(result.error || 'Карта отклонена');
    }
    
  } catch (error) {
    console.error('Payment error:', error);
    showPaymentStatus(`❌ Ошибка: ${error.message}`, 'error');
    submitPayment.disabled = false;
    submitPayment.textContent = '💳 ОПЛАТИТЬ $35';
  }
}
...
function validateForm() {
  ...
  if (!validateCardNumber(cardNumber.value)) {
    showPaymentStatus('❌ Неправильный номер карты', 'error');
    return false;
  }
  
  // Проверка срока действия
  if (!validateExpiry(cardExpiry.value)) {
    showPaymentStatus('❌ Неправильный срок действия', 'error');
    return false;
  }
  
  // Проверка CVC
  if (!validateCvc(cardCvc.value, cardNumber.value)) {
    showPaymentStatus('❌ Неправильный CVC код', 'error');
    return false;
  }
  
  // Проверка имени
  if (!cardName.value.trim()) {
    showPaymentStatus('❌ Введите имя на карте', 'error');
    return false;
  }
...
function completePayment(paymentId) {
  ...
  // Показываем сообщение на домашней странице
  showHomeMessage('✅ Платеж $35 успешен! Уровни 15-35 разблокированы!');
  
  // Обновляем меню уровней если оно открыто
...
function openPaymentModal() {
  ...
  // Фокусируемся на поле номера карты
  setTimeout(() => {
    cardNumber.focus();
  }, 100);
}
...
buyUnlockBtn.onclick = () => {
  if (confirm('РАЗБЛОКИРОВАТЬ УРОВНИ 15-35 ЗА $35?\n\n⚠️ ВНИМАНИЕ: с вашей карты будет списано $35\n💳 Введите реальные данные карты для оплаты')) {
    openPaymentModal();
  }
};
...
/* ============================================
   ПРОВЕРКА СТАТУСА ПОКУПКИ ПРИ ЗАГРУЗКЕ
   ============================================ */
function checkPurchaseStatus() {
  ...
  // Проверяем URL параметры (возврат после оплаты)
...
function completePurchaseFromUrl() {
  ...
  isPremium = true;
  updatePurchaseUI();
  showHomeMessage('✅ Платеж подтвержден! Уровни разблокированы.');
}
